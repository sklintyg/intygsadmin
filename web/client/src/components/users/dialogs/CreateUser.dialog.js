  ...
  return (
    <Modal isOpen={isOpen} size={'md'} backdrop={true} toggle={cancel}>
      ...
      <StyledBody>
        <FormGroup>
          <Label for="userName">
            <IaTypo04>{getMessage(`${messagePrefix}.name`)}</IaTypo04>
          </Label>
          <Input id="userName" value={newUser.name} maxLength={200} onChange={(e) => onChange('name')(e.target.value)} />
        </FormGroup>

        <FormGroup>
          <Label for="userHsaId">
            <IaTypo04>{getMessage(`${messagePrefix}.hsaId`)}</IaTypo04>
          </Label>
