...
describe('intygInfoList actions', () => {
  ...
  describe('fetchIntygInfoList', () => {
    test('success', () => {
      ...
      vi.spyOn(api, 'fetchIntygInfoList').mockResolvedValue(response)
      ...
      return functionToTest(store, () => actions.fetchIntygInfoList({ sortColumn: 'createdAt', sortDirection: 'DESC' }), expectedActions)
    })
    ...
    test('load sort from store', () => {
      ...
      return functionToTest(store, () => actions.fetchIntygInfoList(), expectedActions)
    })

    test('failure', () => {
      const response = [{}]

      vi.spyOn(api, 'fetchIntygInfoList').mockRejectedValue(response)
