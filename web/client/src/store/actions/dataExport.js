...
export const fetchDataExportStatusList = (dataExportId) => (dispatch, getState) => {
  ...
  dispatch({ type: FETCH_DATA_EXPORT_STATUS_LIST_REQUEST });
...
export const removeDataExport = (id) => (dispatch, getState) => {
  ...
  dispatch({ type: REMOVE_DATA_EXPORT_REQUEST });
...
export const updateDataExport = (dataExportId, representativePersonId, representativePhoneNumber) => (dispatch, getState) => {
  if (getIsFetching(getState())) {
    return Promise.resolve();
  }
  ...
  return api.editDataExport(dataExportId, representativePersonId, representativePhoneNumber).then(
    (response) => {
      return dispatch({
