...
class StatusControllerTest {
  ...
  private static final List<NotificationStatusEnum> STATUS_LIST =
      List.of(NotificationStatusEnum.FAILURE);
  private static final String CARE_GIVER_ID = "careGiverId";

  @Mock private SendStatusService sendStatusService;

  @Mock private SendStatusForCertificatesService sendStatusForCertificatesService;

  @Mock SendStatusForUnitsService sendStatusForUnitsService;

  @Mock private SendStatusForCareGiverService sendStatusForCareGiverService;

  @InjectMocks private StatusController statusController;

  @Test
  void shouldSendStatus() {
    final var sendStatusResponse = SendStatusResponseDTO.builder().count(1).build();
  ...
  void shouldCountStatusesForCertificates() {
    ...
    final var countStatusesResponse = CountStatusesDTO.builder().count(1).max(1).build();
  ...
  void shouldSetSendStatusForUnits() {
    final var request =
        SendStatusForUnitsRequestDTO.builder()
  ...
  void shouldCountStatusesForUnits() {
    final var request =
        CountStatusesForUnitsRequestDTO.builder()
            .unitIds(List.of("unitId"))
            .start(START)
            .end(END)
            .statuses(STATUS_LIST)
  ...
  void shouldSetSendStatusForCareGiver() {
    ...
    when(sendStatusForCareGiverService.send(CARE_GIVER_ID, request))
        .thenReturn(sendStatusResponse.getCount());
  ...
  void shouldCountStatusesForCareGiver() {
    ...
    when(sendStatusForCareGiverService.count(CARE_GIVER_ID, request))
        .thenReturn(countStatusesResponse);
