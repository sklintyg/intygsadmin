...
public class RequestErrorController implements ErrorController {
  ...
  public static final String IA_SPRING_SEC_ERROR_CONTROLLER_PATH =
      IA_ERROR_CONTROLLER_PATH + "/spring-sec-error";
  ...
  public ModelAndView handleSpringSecurityErrorAsRedirect(
    ...
    final Throwable error =
        (Throwable) request.getAttribute(WebAttributes.AUTHENTICATION_EXCEPTION);
    ...
    String redirectView =
        "redirect:"
            + IA_CLIENT_EXIT_BOOT_PATH
            + apiErrorResponse.getErrorCode()
            + "/"
            + apiErrorResponse.getLogId();

    log.error(
        String.format(
            "(Page) Spring Security error intercepted: %s, %s - responding with: %s",
            errorContext, apiErrorResponse, redirectView),
        error);
    return new ModelAndView(redirectView);
  }

  /**
   * For normal "browser navigation" initiated requests, we handle all error with a redirect to a
   * specific startup view to present an error.
   *
   * @param request - request
   * @return modelAndView - modelAndView
   */
  ...
  public ModelAndView handleErrorAsRedirect(WebRequest webRequest, HttpServletRequest request) {
    ...
    log.error(
        String.format(
            "(Page) Request error intercepted: %s - responding with: %s",
            errorContext, redirectView));
    return new ModelAndView(redirectView);
  }

  /**
   * For xhr-requests, we handle all errors by responding with a specific error code json struct
   * that client can act on.
   *
   * @param request - request
  ...
  @ResponseBody
  public ResponseEntity<ApiErrorResponse> handleErrorAsJsonResponse(
      WebRequest webRequest, HttpServletRequest request) {
    final HttpStatus httpStatus = getDispatcherErrorStatusCode(request);
    final String errorContext = getErrorContext(webRequest);
    final Throwable error = this.errorAttributes.getError(webRequest);
    ...
    log.error(
        String.format(
            "(REST) Request error intercepted: %s - responding with: %s",
            errorContext, apiErrorResponse),
        error);
  ...
  private ApiErrorResponse fromException(Throwable error) {
    ...
    if (error instanceof IaServiceException) {
      ...
      return new ApiErrorResponse(
          iaException.getErrorCode().name(), error.getMessage(), iaException.getLogId());
