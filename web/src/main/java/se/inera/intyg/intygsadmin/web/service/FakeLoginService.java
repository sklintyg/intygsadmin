...
public class FakeLoginService {
  ...
  public void login(FakeUser fakeUser, HttpServletRequest request) {
    ...
    final var claims =
        Map.<String, Object>of(
            idpProperties.getUserNameAttributeName(), fakeUser.getEmployeeHsaId());
    final var oidcToken =
        new OidcIdToken(FAKE_OIDC_ID_TOKEN, now, now.plus(1, ChronoUnit.HOURS), claims);
    ...
    final var authentication =
        new UsernamePasswordAuthenticationToken(user, oidcToken, Set.of(grantedAuthority));
  ...
  private UserEntity getUserEntity(FakeUser fakeUser) {
    return userPersistenceService
        .findByEmployeeHsaId(fakeUser.getEmployeeHsaId())
        .orElseThrow(
            () ->
                new BadCredentialsException(
                    "Authentication failed. No IntygsadminUser found for employeeHsaId "
